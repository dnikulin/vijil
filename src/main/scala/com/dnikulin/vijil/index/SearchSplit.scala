...
object SearchSplit {
  ...
  def apply(texts: List[TextFile], lexers: List[TextLexer], search: Search): Array[Array[LinkSpanSet]] = {
  ...
  def exact(texts: List[TextFile], lexers: List[TextLexer], size: Int, extent: Int): (Array[LinkSpanSet], Array[LinkSpanSet]) =
    split2(apply(texts, lexers, models => expand(models, SearchSet(models, size), extent)))

  def stencil(texts: List[TextFile], lexers: List[TextLexer], stencil: StencilModel, extent: Int): (Array[LinkSpanSet], Array[LinkSpanSet]) =
