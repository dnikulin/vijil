...
object BZip2ByteStore {
  val blockSize = BZip2CompressorOutputStream.MAX_BLOCKSIZE

  def zipBytes(bytes: Array[Byte]): Array[Byte] = {
    // Compress into memory buffer.
    val byteBuffer = new ByteArrayOutputStream()

    val zipper = new BZip2CompressorOutputStream(byteBuffer, blockSize)
    try {
      zipper.write(bytes)
    } finally {
      zipper.close()
  ...
  def unzipBytes(zip: Array[Byte]): Array[Byte] = {
    ...
    val unzipper = new BZip2CompressorInputStream(ibuffer)

    // Build output buffer from zipped data.
    val obuffer = new ByteArrayOutputStream()
    val pack = new Array[Byte](8192)
    var read = 1
    while (read > 0) {
...
class BZip2ByteStore(override val lower: ObjectStore[Array[Byte]])
  extends ObjectStoreLayer[Array[Byte], Array[Byte]] with ByteStore {

  import BZip2ByteStore._
