...
import com.dnikulin.vijil.tools.ObjectCache

class CachedLexer(val lexer: TextLexer) extends TextLexer {
  override def apply(text: TextFile): TextModel =
    cache(text)

  private val cache = ObjectCache[TextFile, TextModel](lexer)
}
